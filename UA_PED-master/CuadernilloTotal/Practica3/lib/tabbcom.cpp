...
TComplejo *TABBCom::MostrarNiveles(TListaCom &tlcom)
{
  if (tlcom.EsVacia())
  {
    cout << "{}" << endl;
  ...
  for (TListaPos pos = tlcom.Primera(); !pos.EsVacia(); pos = pos.Siguiente())
  {
    if (i % 2 == 0)
    {
      tlcom.Borrar(pos);
  ...
  TComplejo *tcomarr = new TComplejo[tlcom.Longitud()];
  if (tcomarr == NULL)
  {
    cerr << "No se pudo reservar memoria para crear un array de COmplejos en TABBCom:MostrarNiveles:tcomarr";
    return NULL;
  }
  ...
  for (i = 0; i < tlcom.Longitud(); i++)
  {
    this->Insertar(tcomarr[i]);
  }

  for (i = 0; i < tlcom.Longitud(); i++)
  {
    int j;
    cout << "{Elemento: " << tcomarr[i] << ". Elementos de su nivel: ";

    // Hay que restar la mitad de algo, o comprobar con el divisor de la iteracion....
    // 1:1,      2:2, 3:2,      4:4, 5:4, 6:4, 7:4     8:8, 9:8, 10:8, 11:8, 12:8, 13:8, 14:8, 15:8       16:16
    //-------|---------------|-----------------------|-------------------------------------------------|-----------

    for (j = i; j < (i * 2 + 1) - i / 2; j++)
    {
      cout << tcomarr[j];
    }
    cout << "}" << endl;
